...
1. **Detect subject**; `ls` the relevant dirs.
...
3. **Apply spacing + interleaving:**
   - **Spacing** — if a prior `review/<subject>/` file exists, revisit those items *before* new ones.
...
6. **Write** `review/<subject>/<date>.md` (results + weak items).
